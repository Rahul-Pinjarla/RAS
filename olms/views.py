...
def userhome(response):
    ...
    elif response.user.profile.usertype == 'care_taker':
...
def approve(response, loro, id, remark=None):
    ...
    if response.user.is_superuser or not isinstance(response.user, User):
        return redirect('logout')
    if usertype(response) not in ['admin', 'care_taker']:
        print('usertype error')
        return redirect('home')
    if usertype(response) == 'care_taker':
        if loro == 'l':
            ...
            threading.Thread(target=sendMail, args=(leave.user.user.email,
                                                    ...
                                                    f'Dear student your leave regarding {leave.reason} is approved by care taker  \n \n It now only need the approval of your admin \n\n Leave Details:  \n \n Reason : {leave.reason} \n \n Description : {leave.description} \n \n Out date : {leave.out_date} \n \n In date : {leave.in_date}  \n \n Is Emergency : {leave.is_emergency}  \n \n Remark : {leave.remark}')
                            ).start()
        elif loro == 'o':
            ...
            threading.Thread(target=sendMail, args=(outing.user.user.email,
                                                    ...
                                                    f'Dear student your outing regarding {outing.reason} is approved by care taker  \n \n It now only need the approval of your admin \n\n,{outing.date}  \n \n OLMS IIIT Srikakulam')
                            ).start()
        return redirect('care_taker')
    if loro == 'l':
        leave = Leaves.objects.get(id=id)
...
def reject(response, loro, id):
    ...
    if usertype(response) == 'care_taker':
        if loro == 'l':
            ...
            if response.user.profile.hostel != leave.user.hostel or (leave.status != 'pending'):
                return redirect('home')
            leave.status = 'crejected'
            leave.save()
            threading.Thread(target=sendMail, args=(leave.user.user.email,
                                                    f'Your leave is rejected, remark:{leave.remark}',
                                                    f'Dear student your leave regarding {leave.reason} is rejected by your caretaker \n \n OLMS IIIT Srikakulam \n \n Leave Details:  \n \n Reason : {leave.reason} \n \n Description : {leave.description} \n \n Out date : {leave.out_date} \n \n In date : {leave.in_date}  \n \n Is Emergency : {leave.is_emergency}  \n \n Remark : {leave.remark}')
                            ).start()
        elif loro == 'o':
            outing = Outing.objects.get(id=id)
            if response.user.profile.hostel != outing.user.hostel or outing.status != 'pending':
                return redirect('home')
            outing.status = 'crejected'
            outing.save()
            threading.Thread(target=sendMail, args=(outing.user.user.email,
                                                    f'Your outing is rejected, Date:{outing.date}',
                                                    f'Dear student your outing regarding {outing.reason} is rejected by your caretaker  \n \n OLMS IIIT Srikakulam')
                            ).start()
        return redirect('care_taker')
    if loro == 'l':
        leave = Leaves.objects.get(id=id)
...
def care_taker(res):
    if usertype(res)!= 'care_taker':
        return redirect('home')
    leaves = Leaves.objects.filter(
        user__hostel=res.user.profile.hostel, status='pending').order_by('-is_emergency', '-id')
    # if g == 'delayed':
    pinfos = [Personal_info.objects.get(userprofile=l.user) for l in leaves]
    pnb = []
    ppnb = []
    ...
    context = {'leaves': leaves, 'outings': outings, 'username': res.user.username,
                'pnb': pnb, 'ppnb': ppnb}
    print(leaves)
    return render(res, 'caretaker.html', context=context)
